                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($agendas as $agenda)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($agenda->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($agenda->end_time)->format('H:i') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $agenda->title }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $agenda->room->name ?? '-' }}</td>
                ...
                <form id="agendaForm" method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            {{-- === KODE INTEGRASI RUANGAN === --}}
                            <div>
                                <label for="room_id" class="block text-sm font-medium text-gray-700">Pilih Ruangan (Opsional)</label>
                                <select name="room_id" id="room_id" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            ...
            if (mode === 'add') {
                ...
                fetch(`/agenda-harian/${id}`)
                    ...
                    .then(data => {
                        ...
                        // === KODE INTEGRASI RUANGAN (JAVASCRIPT) ===
                        document.getElementById('room_id').value = data.room_id || ''; // Set ke string kosong jika null
                        // ===========================================
