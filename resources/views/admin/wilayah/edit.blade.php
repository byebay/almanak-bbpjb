        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.wilayah.update', $wilayah) }}" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="kode" :value="__('Kode Wilayah (slug)')" />
                            <x-text-input id="kode" name="kode" type="text" class="mt-1 block w-full" :value="old('kode', $wilayah->kode)" required />
                        ...
                        <div>
                            ...
                            <textarea id="informasi" name="informasi" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="4">{{ old('informasi', $wilayah->informasi) }}</textarea>
